...
def register_patient(request):
    if request.method == 'POST':
        ...
        patient = User.objects.filter(username=username).exists()
        if patient:
            messages.error(request,"Username you typed is already exists..Plzzz choose another username or contact admin panel")
            return redirect("/register_patient")
        
        patient_user = User.objects.create_user(username=username, password=password1,)
        patient_user.is_active = True  # Disable login until approved
        patient_user.save()
        ...
        messages.success(request, "You are successfully login to the portal")
